...
void DisplayStringStart(int start, unsigned char position, _far const char * string)
{
	static unsigned char next_pos = 0xFF;
	int i = 0;
...
void DisplayBlinkString(unsigned int delay, int start, int end, unsigned char position, _far const char * string) {
	static unsigned long tmp = 0;
	static int isBlink = 0;
	char str[16] = {0,};
	strcpy(str, string);
	
	if (tmp == 0) tmp = timerCount;
	if (isBlink == 0) {
		if (timerCount - tmp > delay) {
			isBlink = 1;
			tmp = timerCount;
		}
	} else {
	...
	DisplayString(position, str);
}

void DisplayBlinkChar(unsigned int delay, int start, unsigned char position, const char c1, const char c2, const char *string) {
	static unsigned long tmp = 0;
	static int isBlink = 0;
	char str[2] = {0,};
	static unsigned char next_pos = 0xFF;
